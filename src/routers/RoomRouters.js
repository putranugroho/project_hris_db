...
router.post('/booking', (req,res)=>{
    const sql_reservation = `INSERT INTO tb_reservation(reservation_id, pic, jabatan, agenda, time, addon, participant, user_booking, stsrec) VALUES (?,?,?,?,?,?,?,?,0)`
    ...
    const count = `SELECT count_id FROM tb_configurasi`
    const update_reservation = `UPDATE tb_configurasi SET count_id = ?`
    const data = req.body

    let date = data.detail_booking.date.split("-")
    let date_time_slot = date.join("/")
    const a = date[0]
    date[0] = date[2]
    date[2] = date[1]
    date[1] = a
    ...
    conn.query(count, (err, result_count) => {
        if (err) {
            ...
            result_count = parseInt(result_count[0].count_id)+1
            if (result_count < 10) {
            ...
            conn.query(sql_reservation, [reservation_id,data.detail_booking.pic,data.detail_booking.jabatan,data.detail_booking.agenda,data.detail_booking.time,addon,data.detail_booking.nama.length,user_booking],(err, result_reservation) => {
                if (err) {
                    console.log(err);
                    return res.send("error")
                } else {
                    conn.query(update_reservation, reservation_id, (err, result_update) => {
                        if (err) {
                            console.log(err);
                        } else {
                    ...
                    for (let i = 0; i < data.detail_booking.time_slot_id.length; i++) {
                        conn.query(sql_time_slot, [data.detail_booking.ruangan.id,data.detail_booking.time_slot_id[i],reservation_id,date_time_slot], (err, result_participant) => {
                            if (err) {
                                console.log(err);
                            } else {
                                console.log("success");
